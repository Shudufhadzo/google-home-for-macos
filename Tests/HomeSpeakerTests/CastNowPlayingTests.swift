...
final class CastNowPlayingTests: XCTestCase {
    func testSongMetadataKeepsTheLiveAudioURLAndUsesMusicFields() throws {
        ...
        XCTAssertEqual(media["contentId"] as? String, stream.absoluteString)
        ...
        XCTAssertEqual(nextSong.media(at: stream)["contentId"] as? String, stream.absoluteString,
                       "Track metadata changes must retain the same uninterrupted live stream")
    }
}
